...
module.exports = (app) => {
  ...
  app.get("/welcome", auth, (req, res) => {
    return res.status(200).json({ welcome: req.user.email });
  });
  ...
  app.post("/forgot-password", async (req, res) => {
    ...
    try {
      const user = await User.findOne({ email });
      if (user) {
        let reset_token = await bcrypt.hash(user.email, 5);
        ...
        let reset_token_expiry = Date.now() + 2 * 60 * 60 * 1000;
